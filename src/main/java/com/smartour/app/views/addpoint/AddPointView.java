package com.smartour.app.views.addpoint;

import com.smartour.app.data.entity.SampleAddress;
import com.smartour.app.data.service.SampleAddressService;
...
@PageTitle("Add Point")
@Route(value = "add/:sampleAddressID?/:action?(edit)", layout = MainLayout.class)
@RolesAllowed("admin")
public class AddPointView extends Div implements BeforeEnterObserver {

    private final String SAMPLEADDRESS_ID = "sampleAddressID";
    private final String SAMPLEADDRESS_EDIT_ROUTE_TEMPLATE = "add/%d/edit";
    ...
    public AddPointView(@Autowired SampleAddressService sampleAddressService) {
        this.sampleAddressService = sampleAddressService;
        addClassNames("add-point-view", "flex", "flex-col", "h-full");
        ...
        grid.asSingleSelect().addValueChangeListener(event -> {
            if (event.getValue() != null) {
                ...
                clearForm();
                UI.getCurrent().navigate(AddPointView.class);
            }
        });
    ...
    public void beforeEnter(BeforeEnterEvent event) {
        Optional<Integer> sampleAddressId = event.getRouteParameters().getInteger(SAMPLEADDRESS_ID);
        if (sampleAddressId.isPresent()) {
            ...
            if (sampleAddressFromBackend.isPresent()) {
                ...
                // refresh grid
                refreshGrid();
                event.forwardTo(AddPointView.class);
            }
        }
    }
